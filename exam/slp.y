...
%token <ival> NUM
%token <var>  ID READ WRITE IF THEN ELSE ENDIF WHILE DO ENDWHILE INTEGER BOOLEAN DECL ENDDECL BEGINING END MAIN RETURN EQUAL TRUE FALSE GE LE NE AND OR NOT PAIR
...
gidlist   : gid','gidlist                                   {}
          | gid                                             {}
  ;
gid	  : ID                                              {if(datatype==3)
							    	Ginstall($1,datatype,2,"%ID%",NULL);
							     else
								Ginstall($1,datatype,1,"%ID%",NULL);}
...
argidlist : argid','argidlist                               {}
          | argid                                           {}
  ;
argid     : ID                                              {if(datatype==2)Arginstall($1,datatype,1); else Arginstall($1,datatype,2);}
...
lidlist   : lid','lidlist                                   {}
          | lid                                             {}
  ;
lid       : ID						    {Linstall($1,datatype,lbind,1); if(datatype==2 || datatype==1)lbind+=1; else lbind+=2;}
  ;
...
Stmt      : IF'('expr')'THEN Slist ENDIF';'                 {$$=mkCondNode("%IF%",$3,NULL,$6);}
	  ...
	  | ID'.'NUM'='expr';'                              {$$=mkEquPairNode("=",ckLeafNode_Id($1),$3,$5);}
...
expr      : expr'<'expr         			    {$$=mkBoolOptNode("<",$1,$3);}
	  ...
	  | expr OR expr                                    {$$=mkBoolOptBoolNode("OR",$1,$3);}
          ...
          | '('expr')'          	       	            {$$=$2;}
          | ID                  	                    {$$=ckLeafNode_Id($1);}
          | ID'.'NUM                                        {$$=ckLeafNode_Pair_Val($1,$3);}
	  | ID'('exprlist')'                                {$$=ckLeafNode_Function($1,$3);}
	  | '('expr','expr')'                               {$$=ckLeafNode_Pair($2,$4);}
